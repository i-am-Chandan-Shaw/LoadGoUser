...
const Chip = ({ label, selected, onSelect }) => {
  return (
    ...
    >
      <Text style={[style.label, selected && style.selectedLabel]}>{label}</Text>
    </TouchableOpacity>
  );
};
...
const RatingScreen = (props) => {
  ...
  const chipData = ['Overall Service', 'Delivery', , 'Tracking', 'Costs', 'Driver Behavior', 'Safety'];
    ...
    <View style={style.container}>
      <View style={style.topContainer}>
        <View style={style.header}>
          ...
          <Text style={[style.mediumText, { marginTop: 10 }]}> {tripDetails?.driverName}</Text>
        ...
        {/* Chip Container */}
        <Text style={[style.smallText, { fontWeight: 600 }]}> Tell us what can be improved?</Text>
